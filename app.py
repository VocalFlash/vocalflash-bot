...
def whatsapp():
    if request.method == "GET":
        ...
        # accetta VERIFY_TOKEN, WA_VERIFY_TOKEN o ciao123
        verify_token = os.getenv("WA_VERIFY_TOKEN") or os.getenv("VERIFY_TOKEN") or "ciao123"
        if request.args.get("hub.verify_token") == verify_token:
            print(f"WEBHOOK VERIFICATO con token {verify_token}", flush=True)
            return request.args.get("hub.challenge")
        print(f"VERIFY FALLITO: ricevuto {request.args.get('hub.verify_token')} atteso {verify_token}", flush=True)
        return "error", 403
    ...
    try:
        ...
        from_id = msg['from']
        if 'audio' not in msg:
            print(f"Messaggio non audio: {msg.get('type')}", flush=True)
            return "ok", 200

        audio_id = msg['audio']['id']
        token = os.getenv("WA_TOKEN")
        phone_id = os.getenv("WA_PHONE_ID") or os.getenv("PHONE_NUMBER_ID") or "1318571384677144"

        # FIX: chiedi SEMPRE url fresco a Graph API
        print(f"Recupero url per {audio_id}", flush=True)
        r = requests.get(f"https://graph.facebook.com/v20.0/{audio_id}", headers={"Authorization": f"Bearer {token}"})
        print(f"Media info: {r.text}", flush=True)
        r.raise_for_status()
        media_url = r.json()['url']

        print(f"Scarico audio...", flush=True)
        audio_resp = requests.get(media_url, headers={"Authorization": f"Bearer {token}"})
        audio_resp.raise_for_status()
        with open("/tmp/audio.ogg","wb") as f:
            f.write(audio_resp.content)
        print(f"Audio {len(audio_resp.content)} bytes salvato", flush=True)

        # FIX: supporta sia OPENAI_API_KEY che OPENAI_KEY
        ...
        if not openai_key or not openai_key.startswith("sk-"):
            ...
            with open("/tmp/audio.ogg","rb") as f:
                ...
                transcript = tr.text
                lingua = tr.language
            print(f"Trascrizione: {transcript} [{lingua}]", flush=True)
            if lingua!= "it":
                trad = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[{"role":"system","content":"Traduci in italiano naturale"},{"role":"user","content":f"{transcript}"}]
        ...
        print(f"Risposta: {resp.status_code} {resp.text}", flush=True)
